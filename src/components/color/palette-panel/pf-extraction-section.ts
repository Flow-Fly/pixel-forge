...
export class PFExtractionSection extends BaseComponent {
  ...
  private async handleExtract() {
    await this.palette.extractFromDrawing();
  }

  private handleColorClick(color: string) {
    this.palette.addExtractedColor(color);
  }
  ...
  private addAll() {
    this.palette.addAllExtracted();
  }

  private replaceAll() {
    this.palette.replaceWithExtracted();
  }

  render() {
    const extractedColors = this.palette.extractedColors.value;
    const isExtracting = this.palette.isExtracting.value;
    ...
    return html`
      ...
      ${
        this.expanded
          ? html`
              <div class="content">
                <button class="extract-btn" @click=${this.handleExtract} ?disabled=${isExtracting}>
