...
import { getLightnessVariations } from './palette/variations';

interface LightnessPalette {
  getLightnessVariations(color: string): string[];
}

const defaultLightnessPalette: LightnessPalette = {
...
class ColorStore {
  ...
  constructor(palette: LightnessPalette = defaultLightnessPalette) {
